...
function MediumButton(options) {
	...
	this.button.onclick = function () {
		...
		if (options.start === undefined || html.indexOf(options.start) == -1 && html.indexOf(options.end) == -1) {
			...
			if (options.action != undefined) html = options.action(html, true, parent, sel);
			html = options.start + html + options.end;
		}
		else { // clean old

			if (options.action != undefined) html = options.action(html, false, parent, sel);
